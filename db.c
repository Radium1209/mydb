...
uint32_t ID_SIZE, ID_OFFSET;
uint32_t USERNAME_SIZE, USERNAME_OFFSET;
uint32_t EMAIL_SIZE, EMAIL_OFFSET;
uint32_t ROW_SIZE, PAGE_SIZE;
 ...
 */
uint32_t NODE_TYPE_SIZE, NODE_TYPE_OFFSET;
uint32_t IS_ROOT_SIZE, IS_ROOT_OFFSET;
uint32_t PARENT_POINTER_SIZE, PARENT_POINTER_OFFSET;
uint8_t COMMON_NODE_HEADER_SIZE;

/*
 * Leaf Node Header Layout
 */
uint32_t LEAF_NODE_NUM_CELLS_SIZE, LEAF_NODE_NUM_CELLS_OFFSET;
uint32_t LEAF_NODE_HEADER_SIZE;

/*
 * Leaf Node Body Layout
 */
uint32_t LEAF_NODE_KEY_SIZE, LEAF_NODE_KEY_OFFSET;
uint32_t LEAF_NODE_VALUE_SIZE, LEAF_NODE_VALUE_OFFSET;
 ...
 */
uint32_t INTERNAL_NODE_NUM_KEYS_SIZE;
uint32_t INTERNAL_NODE_NUM_KEYS_OFFSET;
uint32_t INTERNAL_NODE_RIGHT_CHILD_SIZE;
uint32_t INTERNAL_NODE_RIGHT_CHILD_OFFSET;
uint32_t INTERNAL_NODE_HEADER_SIZE;

/*
 * Internal Node Body Layout
 */
uint32_t INTERNAL_NODE_KEY_SIZE;
uint32_t INTERNAL_NODE_CHILD_SIZE;
uint32_t INTERNAL_NODE_CELL_SIZE;
...
uint32_t* leaf_node_cell(void* node, uint32_t cell_num) {
    return (uint32_t*)(node + LEAF_NODE_HEADER_SIZE + cell_num * LEAF_NODE_NUM_CELLS_SIZE);
}
...
Cursor* table_find(Table* table, uint32_t key) {
    ...
    if (get_node_type(root_node) == NODE_LEAF) {
        ...
        printf("Noeed to implement searching an internal node\n");
        exit(EXIT_FAILURE);
    }
}
